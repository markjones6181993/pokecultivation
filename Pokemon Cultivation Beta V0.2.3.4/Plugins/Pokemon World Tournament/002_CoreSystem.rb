...
# will not show up on your tournament selection list.
  ...
  def initialize(viewport)
    ...
    @challenge_rules = GameData::PWTTournament.get(@tournament_id).call_rules([6,6,6,6][@battle_type])
  ...
  def chooseBattle
    ...
    choices = [_INTL("Full"),_INTL("Cancel")]
  ...
  def introduction
    ...
    pbMessage(_INTL("The place where the strongest gather to compete, powered by Macro Cosmos!.\\1"))
  ...
  def startTournament
    ...
    pbMessage(_INTL("Announcer: It's time to lock in and duke it out for the title of \"World Champion!\".\\1"))
    pbMessage(_INTL("Announcer: Take a look up at our scoreboard. Let's see who we have competing today!\\1"))
    trainer = self.generateRound1
    self.displayScoreboard(trainer)
    self.moveSwitch('A')
    pbMessage(_INTL("Announcer: Without further ado... LET'S GET READY TO RUMMMMMMMBLE!\\1"))
    pbMessage(_INTL("Announcer: {1} and {2} are ready to go! It's Battle Time in Verdanturf City!",$player.name,trainer.name))
    ...
    if pbPWTBattle(trainer,@challenge_rules,@outcome)
      ...
      @beat.push(trainer)
      pbMessage(_INTL("Announcer: Unbelievable! What an incredible first round!\\1"))
      ...
      pbMessage(_INTL("Announcer: Let's take another look at that scoreboard for our results.\\1"))
      trainer = self.generateRound2
      self.displayScoreboard(trainer)
      pbMessage(_INTL("Next up, we've got {1} and {2}. Two powerhouse Trainers squaring off!\\1",$player.name,trainer.name))
      self.visualRound(trainer)
      pbMessage(_INTL("Announcer: Let's go! Trainers, ready your Pokémon!"))
      pbMessage(trainer.beforebattle) if !trainer.beforebattle.nil?
      if pbPWTBattle(trainer,@challenge_rules,@outcome)
        @round = 2
        pbMessage(trainer.afterbattle) if !trainer.afterbattle.nil?
        ...
        pbMessage(_INTL("Announcer: These trainers are really giving it their all.\\1"))
        self.visualRound(trainer,true)
        pbMessage(_INTL("Announcer: And finally... Our crowning match has been decided!\\1"))
        ...
        self.visualRound(trainer)
        pbMessage(_INTL("Announcer: Ready? Fight!"))
        pbMessage(trainer.beforebattle) if !trainer.beforebattle.nil?
        if pbPWTBattle(trainer,@challenge_rules,@outcome)
          pbBGMFade(0.1)
		  @round = 3
          ...
          pbMessage(_INTL("Announcer: What an incredible series of matches here in Verdanturf Colosseum - brought to you by Macro Cosmos!"))
          return "win"
        end
      end
    end
...
def pbLoadTrainer(*args)
  ...
  party = trainer.party
  # For full tournament options (Single, Double, Full, Sudden Death)
  #length = [3,4,6,1][$PWT.battle_type]
  length = [6,6,6,6][$PWT.battle_type]
