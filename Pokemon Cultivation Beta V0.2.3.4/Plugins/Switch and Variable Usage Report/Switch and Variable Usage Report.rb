...
def pbFindSwitchVariable(search_id, type = :Switch, print = :Console)
      ...
      pages.each_with_index do |page, page_id|
        ...
        list.each_with_index do |cmd, i|
          ...
          when 111 # Conditional Branch            
            ...
            case type
            when :Switch
              list_strings.push("        [Line #{line_number}] Conditional Branch Switch") if cmd.parameters[0] == 0 && cmd.parameters[1] == search_id
              if cmd.parameters[0] == 12 && cmd.parameters[1].include?("$game_switches[")
                ...
                list_strings.push("        [Line #{line_number}] Conditional Branch Script command: $game_switches") if values.include?(search_id)
              end
            when :Variable
              list_strings.push("        [Line #{line_number}] Conditional Branch Variable") if cmd.parameters[0] == 1 && cmd.parameters[1] == search_id
              if cmd.parameters[0] == 12
                ...
                values = cmd.parameters[1].scan(/\$game_variables\[(\d+)\]/i).flatten.map!(&:to_i)
                  list_strings.push("        [Line #{line_number}] Conditional Branch Script command: $game_variables") if values.include?(search_id)
                end
                if cmd.parameters[1].include?("pbGet(")
                  values = cmd.parameters[1].scan(/pbGet\((\d+)\)/i).flatten.map!(&:to_i)
                  list_strings.push("        [Line #{line_number}] Conditional Branch Script command: pbGet") if values.include?(search_id)
                end
                if cmd.parameters[1].include?("pbSet(")
                  values = cmd.parameters[1].scan(/pbSet\((\d+),/i).flatten.map!(&:to_i)
                  list_strings.push("        [Line #{line_number}] Conditional Branch Script command: pbSet") if values.include?(search_id)
                end
              end
            end
          when 121 # Switches
            if type == :Switch
              if cmd.parameters[0] == cmd.parameters[1]
                list_strings.push("        [Line #{line_number}] Control Switches command") if cmd.parameters[0] == search_id
              else
                list_strings.push("        [Line #{line_number}] Control Switches command (Batch)") if search_id.between?(cmd.parameters[0], cmd.parameters[1])
              end
            end
          when 122 # Variables
            if type == :Variable
              if cmd.parameters[0] == cmd.parameters[1]
                list_strings.push("        [Line #{line_number}] Control Variables command") if cmd.parameters[0] == search_id
              else
                list_strings.push("        [Line #{line_number}] Control Variables command (Batch)") if search_id.between?(cmd.parameters[0], cmd.parameters[1])
              end
            end
          when 355,655 # Script
            if type == :Switch && cmd.parameters[0].include?("$game_switches[")
              values = cmd.parameters[0].scan(/\$game_switches\[(\d+)\]/i).flatten.map!(&:to_i)
              list_strings.push("        [Line #{line_number}] Script command: $game_switches") if values.include?(search_id)
            end
            if type == :Variable
              if cmd.parameters[0].include?("$game_variables[")
                values = cmd.parameters[0].scan(/\$game_variables\[(\d+)\]/i).flatten.map!(&:to_i)
                list_strings.push("        [Line #{line_number}] Script command: $game_variables") if values.include?(search_id)
              end
              if cmd.parameters[0].include?("pbGet(")
                values = cmd.parameters[0].scan(/pbGet\((\d+)\)/i).flatten.map!(&:to_i)
                list_strings.push("        [Line #{line_number}] Script command: pbGet") if values.include?(search_id)
              end
              if cmd.parameters[0].include?("pbSet(")
                values = cmd.parameters[0].scan(/pbSet\((\d+),/i).flatten.map!(&:to_i)
                list_strings.push("        [Line #{line_number}] Script command: pbSet") if values.include?(search_id)
              end
            end
          end
        end
  ...
  when :File
    ...
    pbMessage(_INTL("Results printed to {1} in the project folder.", path))
	return true
  end
...
MenuHandlers.add(:debug_menu, :switch_usage_report, {
  "name"        => _INTL("Switch Usage Report"),
  "parent"      => :field_menu,
  "description" => _INTL("Find which events use a specific switch."),
  "effect"      => proc {
	commands = []
	search_id = -1
...
MenuHandlers.add(:debug_menu, :variable_usage_report, {
  "name"        => _INTL("Variable Usage Report"),
  "parent"      => :field_menu,
  "description" => _INTL("Find which events use a specific variable."),
  "effect"      => proc {
	commands = []
	search_id = -1
	...
	next false if commands.empty?
	ret = pbMessage(_INTL("Search for which variable?"), commands, -1,  nil, cmd)
	if ret >= 0
